            ...
            <div className={`common-card_wrap flex flex-${direction} shadow rounded !p-5 gap-3`}>
                {title && (
                    <div
                        ...
                        <div
                            className={`common-card_icon flex flex-${direction === 'col' ? 'row' : 'col'} items-center gap-2`}>
                            {icon && (
                                <Icon className={`icon-${icon?.toLowerCase()}`} name={icon} width={24} height={24}
                                      fill={color}/>
                            )}
                            <div
                                className={`common-card_title text-[#333333] text-[18px] font-semibold tracking-wider flex leading-8 items-center`}>{title}</div>
                        </div>
